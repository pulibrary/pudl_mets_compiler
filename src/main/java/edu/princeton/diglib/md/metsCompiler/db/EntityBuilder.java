...
//	public static void main(String[] args) throws XMLStreamException, MissingURIException, UnsupportedNamespaceException, MissingTypeException {
//	    File f = new File("/home/jstroop/workspace/pudl-data-dev/mdata/pudl0032/ns1113.mods");
...
//	}
	...
	public PUDLMETSEntity build(File thisFile) throws XMLStreamException,
			MissingURIException, UnsupportedNamespaceException,
	...
	private static void entityFromTEI(PUDLMETSEntity entity)
			throws XMLStreamException, MissingURIException {
		while (xmlr.hasNext() && entity.getUri() == null) {
			...
			if (e == XMLEvent.START_ELEMENT && xmlr.getLocalName() == "idno") {
				...
				if (xmlr.getText().startsWith("http://"))
	...
	private static void entityFromEAD(PUDLMETSEntity entity)
			throws XMLStreamException, MissingURIException {
		while (xmlr.hasNext() && entity.getUri() == null) {
			int e = xmlr.next();
	...
	// untested
	private static void entityFromMETS(PUDLMETSEntity entity)
			throws MissingURIException, XMLStreamException,
			MissingTypeException {
		...
		while (xmlr.hasNext()
				&& (entity.getType() == null || entity.getUri() == null)) {
			int e = xmlr.next();
			// URI
			if (e == XMLEvent.START_ELEMENT
					&& xmlr.getLocalName() == "altRecordID") {
				xmlr.next();
				entity.setUri(xmlr.getText());
			}
		}
		...
		if (entity.getType() == null)
			throw new MissingTypeException(entity.getPath()
					+ "is missing a @TYPE");
	}

	private static void entityFromMODS(PUDLMETSEntity entity)
			throws XMLStreamException, MissingURIException {
		entity.setType(TYPE.DESCRIPTIVE);
		while (xmlr.hasNext() && entity.getUri() == null) {
			int e = xmlr.next();
			if (e == XMLEvent.START_ELEMENT
					&& xmlr.getLocalName() == "recordIdentifier") {
	...
	private static void entityFromVRA(PUDLMETSEntity entity)
			throws XMLStreamException, MissingURIException {
		entity.setType(TYPE.DESCRIPTIVE);
		while (xmlr.hasNext() && entity.getUri() == null) {
			int e = xmlr.next();
